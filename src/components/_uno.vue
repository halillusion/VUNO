...
export default {
  ...
  methods: {
     ...
     */
    dealCards() {
      [...Array(parseInt(this.numberOfPlayer)).keys()].forEach(number => {
        ...
        for (let card = 0; card < 7; card++) {
          this.playerData[number]["cards"].push(this.drawCard())
        }
      })
  ...
  computed: {
    getTopCards() {
      return this.playerData[this.orderRelation.top].cards
    },
    getBottomCards() {
      return this.playerData[this.orderRelation.bottom].cards
    },
    getLeftCards() {
      return this.playerData[this.orderRelation.left].cards
    },
    getRightCards() {
      return this.playerData[this.orderRelation.right].cards
    }
  },
  mounted() {
...
<template>
  <div class="wrap">
    ...
    <div class="wrap--game">
      <div class="wrap--game-intro" :class="screen == 'intro' ? 'active' : ''">
        <div class="container">
          <div class="row justify-content-center align-items-center vh-100">
            <div class="col-12 col-md-6 col-lg-4">
              ...
              <div class="mt-5">
                ...
                <div class="d-grid">
                  <button @click="prepareGame()" class="btn btn-success d-block btn-lg">Play!</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="wrap--game-counter" :class="screen == 'counter' ? 'active' : ''">
        <div class="container">
          <div class="row justify-content-center align-items-center vh-100">
            <div class="col-12 col-md-6 col-lg-4">
              <div class="count">{{ preparingSecond }}</div>
           </div>
          </div>
        </div>
      </div>
      <div  class="wrap--game-main" :class="screen == 'game' ? 'active' : ''">
        <div class="container">
          <div class="position-relative wrap--game-main-area">
            <div v-if="orderRelation.top !== null" class="position-absolute top-0 start-50 translate-middle top" :class="order == 'top' ? 'current' : ''">
            ...
            <div v-if="orderRelation.left !== null" class="position-absolute top-50 start-0 translate-middle left" :class="order == 'left' ? 'current' : ''">
              <div class="player-side">
                <div class="player-info">
                  <span class="score-badge" data-bs-toggle="tooltip" title="Score">
            ...
            <div class="position-absolute top-50 start-50 translate-middle center">
              <div class="wrap--game-main-area-table" :class="'direction-' + direction + ' ' + color + ' ' + order">
                <div class="other-cards" data-bs-toggle="tooltip" data-bs-placement="top" title="Draw a card" @click="drawACard()">
                  <img src="@/assets/logo.svg" alt="Logo" />
                </div>
                <div class="current-card">
                  <div class="cards">
                  <div v-for="(card, key) in cardsPlayed" :key="key" class="game-card" :data-card-type="card" @click="playerData[0].score+=100">
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div  v-if="orderRelation.right !== null" class="position-absolute top-50 start-100 translate-middle right" :class="order == 'right' ? 'current' : ''">
              <div class="player-side">
                <div class="player-info">
                  <span class="score-badge" data-bs-toggle="tooltip" title="Score">
            ...
            <div class="position-absolute top-100 start-50 translate-middle bottom"  :class="order == 'bottom' ? 'current' : ''">
              <div class="player-side">
                <div class="player-info">
                  <span class="score-badge" data-bs-toggle="tooltip" title="Score">
                    {{ playerData[0].score - 500 }}
                  </span>
                  <span class="name" @click="playerData[0].cards.shift()">{{ playerData[0].name }}</span>
                  <span class="action-buttons">
                    <button class="active" @click="playerData[0].cards.push('yellow:1')">PAS</button>
      ...
      <!--
      <div v-if="screen == 'result'" class="wrap--game-result">
        <img src="@/assets/logo.svg" alt="VUNO Logo" />
      </div>
      -->
    </div>
  </div>
